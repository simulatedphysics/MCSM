...
void demo_basic(std::unique_ptr<Model> & model_ptr) {
    ...
    for(int i = 0; i < ny; i++)
        gp << std::string("set arrow ") + std::to_string(i + nx + 1) + " from " + std::to_string(i) + ",0 to " + std::to_string(i) + "," + std::to_string(ny - 1) + " nohead ls 1\n";
    ...
    for (int i = 0; i < nx*ny; i++) {
        ...
        gp << "set object " + std::to_string(i+1) + " circle at " + std::to_string(x_coord) + "," + std::to_string(y_coord) + " size first 0.15 fc rgb " + col + " front\n";
    }
